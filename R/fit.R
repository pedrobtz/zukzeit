...
# matrix (see ?`zuk-architecture-contract`). This file is one of several
# optional surfaces onto it; hardhat and parsnip are Suggests, and the engine
# works fully without them. Peers: as_fable.zuk_forecast() for the tidyverts,
# and forecast.zuk_model() for plain data frames and tsibbles.
#
...
# reason to supply at forecast time. Keeping it out means `zuk_fit()` needs no
...
#' fit <- zuk_fit(sales ~ ., data = train, model = model,
#'                 index = "day", id = "store")
#' fit
#'
#' # Or name the columns directly.
#' zuk_fit(train, model = model, target = "sales", index = "day", id = "store")
#'
#' zuk_unload("stub")
zuk_fit <- function(x, ...) {
  UseMethod("zuk_fit")
...
#' @param model A `zuk_model` from [zuk_pretrained()].
...
#' @rdname zuk_fit
#' @export
zuk_fit.formula <- function(x, data, model, index, id = NULL,
                             quantile_levels = c(0.1, 0.5, 0.9), ...) {
  if (!inherits(model, "zuk_model")) {
    zuk_abort_contract(
      "{.arg model} must be a {.cls zuk_model} (see {.fn zuk_pretrained}).",
      contract = "fit input",
      expected = "zuk_model",
      actual = class(model)
    )
  ...
  zuk_fit_data_frame(data, model, target = targets, index = index, id = id,
...
#' @rdname zuk_fit
#' @export
zuk_fit.data.frame <- function(x, model, target, index, id = NULL,
                                quantile_levels = c(0.1, 0.5, 0.9), ...) {
  zuk_fit_data_frame(x, model, target = target, index = index, id = id,
                      quantile_levels = quantile_levels)
}

zuk_fit_data_frame <- function(data, model, target, index, id,
                                quantile_levels) {
  if (!inherits(model, "zuk_model")) {
    zuk_abort_contract(
      "{.arg model} must be a {.cls zuk_model} (see {.fn zuk_pretrained}).",
...
#' @export
print.zuk_fit <- function(x, ...) {
  cli::cli_text("{.cls zuk_fit} ({.val {x$model$architecture}})")
...
#'
#' zuk_unload("stub")
predict.zuk_fit <- function(object, new_data, ...) {
  index <- object$index
  id <- object$id
  ...
  # See zuk_fit_data_frame(): keys are labels. Without this, a factor `id`
