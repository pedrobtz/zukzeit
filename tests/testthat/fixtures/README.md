...
Six fixtures in `toto/`, generated by `.agents/generate-toto-reference.py`
against `DataDog/toto@46bd92ad` and checkpoint revision `8306a980`. The
generator pins the raw `Toto2Model.forecast()` knobs -- `decode_block_size = 0`,
`scaler_fallback_min_obs = 0`, `quantile_real_cap_k = 0` -- because the GluonTS
wrapper defaults differ on all three and fixtures generated under them are not
comparable. Each fixture records those settings.

`typical`, `short_context`, and `long_horizon` cover the ordinary regimes.
`mixed_sign` crosses zero and ends below it, so the negative branch of the
`sinh` inverse has a reference attached. `flat_context` has zero variance and
reaches the causal scaler's `minimum_scale` clamp, which a smooth series never
does. `batch_agreement` holds two series of *different* lengths: upstream
cannot batch those and generated each alone, so the port has to pad them into
one batch and land on the same numbers. That case is what caught the missing
unobserved-patch attention mask.

Upstream requires patch-aligned contexts and raises otherwise, so every length
